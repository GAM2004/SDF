...
CREATE TABLE [dbo].[registro_ventas](
    ...
    FOREIGN KEY (factura_id) REFERENCES facturas(id) ON DELETE CASCADE, -- ON DELETE CASCADE para el detalle
...
CREATE TABLE [dbo].[detalle_compras](
    ...
    FOREIGN KEY (compra_id) REFERENCES registro_compras(id) ON DELETE CASCADE, -- ON DELETE CASCADE para el detalle
    FOREIGN KEY (inventario_id) REFERENCES inventario(id)
);
...
CREATE TABLE [dbo].[movimientos_stock](
    ...
    [cantidad] [int] NOT NULL,
    [id_usuario] [int] NOT NULL,
    [referencia_id] [int] NULL, -- Puede ser factura_id, compra_id, etc.
    FOREIGN KEY (inventario_id) REFERENCES inventario(id), -- No eliminar en cascada para mantener historial
    FOREIGN KEY (id_usuario) REFERENCES usuarios(id)
);

-- CORRECCIÓN: Añadida tabla para devoluciones
CREATE TABLE [dbo].[devoluciones](
    [id] [int] IDENTITY(1,1) PRIMARY KEY,
    [venta_id] [int] NOT NULL, -- Referencia a la venta original en registro_ventas
...
CREATE PROCEDURE [dbo].[sp_GetAllProducts] AS BEGIN SELECT p.id, p.nombre, p.precio, (SELECT TOP 1 url_imagen FROM imagenes_producto WHERE producto_id = p.id) as img, (SELECT ISNULL(SUM(existencia), 0) FROM inventario WHERE producto_id = p.id) as stock_total FROM productos p; END
GO
...
GO
CREATE PROCEDURE [dbo].[sp_DeleteProduct] @id INT AS BEGIN DELETE FROM productos WHERE id = @id; END
GO
...
BEGIN
    ...
    BEGIN
        ...
        -- Lanzar un error claro al frontend
    ...
    BEGIN TRY
        ...
        INSERT INTO movimientos_stock (inventario_id, tipo_movimiento, cantidad, id_usuario, referencia_id)
        SELECT
            v.inventario_id,
            'salida por venta',
            -v.cantidad_solicitada,
...
CREATE PROCEDURE [dbo].[sp_GetAllRecords] AS BEGIN SELECT f.id AS factura_id, f.fecha_creacion, ISNULL(f.cliente_nombre, 'N/A') AS cliente_nombre, u.nombre AS nombre_usuario, p.nombre AS nombre_producto, t.nombre AS talla, c.nombre AS color, rv.cantidad, rv.precio_unitario, rv.total_linea FROM registro_ventas rv JOIN facturas f ON rv.factura_id = f.id JOIN inventario i ON rv.inventario_id = i.id JOIN productos p ON i.producto_id = p.id JOIN tallas t ON i.talla_id = t.id JOIN colores c ON i.color_id = c.id JOIN usuarios u ON f.id_usuario = u.id ORDER BY f.fecha_creacion DESC; END
GO
...
-- ================================================================================
-- CORRECCIÓN: Procedimiento de Registrar Compra con Transacción y DROP/CREATE
-- ================================================================================
...
GO
-- ================================================================================
-- CORRECCIÓN: Añadido procedimiento para registrar devoluciones
-- ================================================================================
...
CREATE PROCEDURE [dbo].[sp_RegisterReturn]
    @venta_id INT, -- ID de la tabla registro_ventas
...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO movimientos_stock (inventario_id, tipo_movimiento, cantidad, id_usuario, referencia_id)
        VALUES (@inventario_id, 'entrada por devolución', @cantidad_devuelta, @id_usuario, @devolucion_id);
...
GO
CREATE PROCEDURE [dbo].[sp_GetStockMovements] AS BEGIN SELECT m.id, m.fecha, p.nombre AS nombre_producto, t.nombre AS talla, c.nombre AS color, m.tipo_movimiento, m.cantidad, u.nombre AS nombre_usuario, m.referencia_id FROM movimientos_stock m JOIN inventario i ON m.inventario_id = i.id JOIN productos p ON i.producto_id = p.id JOIN tallas t ON i.talla_id = t.id JOIN colores c ON i.color_id = c.id JOIN usuarios u ON m.id_usuario = u.id ORDER BY m.fecha DESC; END
GO
